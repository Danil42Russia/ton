    ...
    for (size_t i = 0; i < G.source_map.size(); ++i) {
...

      {
...

        {
          ...
          } else if (entry.ast_kind == "ast_int_const" && entry.descr == "thrown-code") {
            desc_builder("description", entry.descr);
            desc_builder("is_assert_thrown_code", td::JsonBool(true));
          }
        ...
        if (entry.is_entry) {
          ...
          event_function = entry.entry_or_leave_name;
        } else if (entry.is_leave) {
          event_type = "LeaveFunction";
          event_function = entry.entry_or_leave_name;
        }  else if (entry.is_after_function_call) {
          event_type = "AfterFunctionCall";
          event_function = entry.entry_or_leave_name;
        ...
        if (event_type != "" && event_function != "") {
          ctx_builder("event", event_type);
          ctx_builder("event_function", event_function);
        }
