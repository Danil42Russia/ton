...
bool Transaction::compute_phase_step_debug(const ComputePhaseConfig& cfg, std::unique_ptr<vm::VmState>& vm,
  ...
  return get_compute_phase_result(cfg, cp, *vm, logger, {}, td::RealCpuTimer::Time{});
}
...
int Transaction::try_action_send_msg(const vm::CellSlice& cs0, ActionPhase& ap, const ActionPhaseConfig& cfg,
  ...
  if (!ext_msg) {
    ...
    if (cfg.global_version < 12) {
      CHECK(block::tlb::t_Grams.pack_integer(info.extra_flags, ihr_fee));
    }
...
bool Transaction::prepare_bounce_phase(const ActionPhaseConfig& cfg) {
  ...
  if (cb.can_extend_by(1 + body.size(), body.size_refs())) {
    ...
    CHECK(cb.store_bool_bool(true) && cb.store_builder_ref_bool(std::move(body)));
  }
